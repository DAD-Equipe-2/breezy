...
export default function Post({ postId }) {
  const [post, setPost] = useState({
    user : {
      ...
      pseudo: "@Pseudo",
      profilePicture: "/profil_picture.jpg",
    },
    date : "17/06/2025",
    imageUrl: "/paysage.jpg",
    ...
    <div className="flex post bg-foreground-500 text-primary p-4 rounded-lg border border-foreground shadow-md mb-2">
      ...
      <div className="flex flex-col ml-1 pr-2 space-y-2">
        <div className="flex whitespace-nowrap space-x-2">
          <h2 className="text-foreground font-bold text-sm font-roboto">{post.user.username}</h2>
          <h3 className="text-secondary text-sm font-roboto">{post.user.pseudo}</h3>
          <p className="text-secondary text-sm font-roboto">{post.date}</p>
        </div>
        <div>
        ...
        <div className="flex space-x-16">
          ...
          <div className="flex space-x-1">
            <button>
              {/* Icône 2 */}
              <svg className="text-foreground" stroke={postInfo.liked ? "red" : "currentColor"} strokeWidth={2} width="1.5rem" height="1.5rem" viewBox="0 0 24 24" fill={postInfo.liked ? "red" : "none"} xmlns="http://www.w3.org/2000/svg">
